...
fn binding_with_flags_and_list() {
    let stmts =
        parse(r#"let r = fs.remove --paths=["/tmp/a", "/tmp/b"] --recursive=true"#).unwrap();
    let Statement::Binding { name, pipeline } = &stmts[0] else {
...
fn try_sets_tried() {
    ...
    let Statement::Binding { pipeline, .. } = &stmts[0] else {
        panic!()
    };
    assert!(pipeline.tried);
}
...
fn value_fed_pipeline() {
    ...
    let Statement::Pipeline(p) = &stmts[0] else {
        panic!()
    };
...
"#;
    ...
    assert_eq!(
        condition.left,
        Value::Var(vec!["r".into(), "status".into()])
    );
    assert_eq!(
        condition.comparison,
        Some((CompareOp::Eq, Value::Str("error".into())))
    );
...
fn lone_value_statement_is_rejected() {
    ...
    assert!(
        err.message.contains("pipe it into a command"),
        "got: {}",
        err.message
    );
}
...
fn negative_and_float_numbers() {
    ...
    assert_eq!(
        p.commands[0].args[0],
        Argument::Positional(Value::Number(-1.0))
    );
    assert_eq!(
        p.commands[0].args[1],
        Argument::Positional(Value::Number(2.5))
    );
}
...
fn unquoted_path_gets_a_quoting_hint() {
    ...
    assert!(
        err.message.contains("must be quoted"),
        "got: {}",
        err.message
    );
}
